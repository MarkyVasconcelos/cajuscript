...
public class CajuScript {
	 ...
	 */
	public static final String CAJU_VARS_STATIC_STRING = CAJU_VARS
			.concat("_static_string_");
	 ...
	 */
	public static final String CAJU_VARS_PARAMETER = CAJU_VARS
			.concat("_param_");
		...
		try {
			...
			lines: for (String line : lines) {
				...
				if (config) {
					while (true) {
						...
						if (configLine.startsWith("caju.syntax")) {
							String syntaxName = configLine.substring(configLine
									.lastIndexOf(' ') + 1);
							...
							if (cacheParser != null
								...
								.startsWith("caju.compile.baseDirectory")) {
							set(CAJU_VAR_COMPILE_BASEDIRECTORY, configLine
									.substring(configLine.lastIndexOf(' ') + 1)
									.trim());
						} else if (configLine
								.startsWith("caju.compile.classPath")) {
							set(CAJU_VAR_COMPILE_CLASSPATH, configLine
									.substring(configLine.lastIndexOf(' ') + 1)
									.trim());
				...
				if (!config && cacheParser != null
					...
					Value finalValue = parserBase
							.execute(this, context, syntax);
					parserBase.clear();
					return finalValue;
				}
					...
					case '\'':
						if (cO != '\\' && !isString2) {
							if (isString1) {
								isString1 = false;
								context
										.setVar(staticStringKey, new Value(
								...
								if (staticContexts != null) {
									staticContexts.setVar(staticStringKey,
											new Value(null, null, null, "'"
													.concat(staticStringValue)
													.concat("'")));
								}
								line = line.replace(
										(CharSequence) ("'"
												.concat(staticStringValue)
												.concat("'")), staticStringKey);
								staticStringKey = "";
								staticStringValue = "";
							} else {
								isString1 = true;
								...
								staticStringKey = CAJU_VARS_STATIC_STRING
										.concat(nextVarsCounter())
										.concat(
												Long
														.toString(staticVarsStringCounter));
								staticVarsStringCounter++;
							}
						} else if (isString2 || cO == '\\') {
							staticStringValue = staticStringValue
			...
			for (String line : lines) {
				...
				if (line.startsWith(LINE_DETAIL_START)) {
					lineN = line
							.substring(0, line.indexOf(LINE_DETAIL_END) + 1);
				...
				while (true) {
					...
					if (p > -1) {
						scriptBuffer.append(lineN + line.substring(0, p)
								+ SUBLINE_LIMITER);
	...
	private int endLineIndex(String line, Syntax syntax) {
		...
		int id = -1;
		for (int i = 0; i < starts.length; i++) {
			if ((id == -1 && ends[i] < Integer.MAX_VALUE)
					|| (ends[i] < Integer.MAX_VALUE && id > -1 && (ends[i] <= starts[id] || (starts[i] < starts[id] && ends[id] == ends[i])))) {
				id = i;
			}
		}
		if (id > -1) {
	 ...
	 * @param type
	 *            Types: "int" or "i", "long" or "l", "double" or "d", "float"
	 *            or "f", "char" or "c", "boolean" or "b", "byte" or "bt",
	...
	public Object cast(Object value, String type) throws Exception {
		if (type.equalsIgnoreCase("int")
			...
			return Integer.valueOf((int) Double.valueOf(value.toString())
					.doubleValue());
		}
		if (type.equalsIgnoreCase("long")
				|| type.equalsIgnoreCase("java.lang.Long")
				|| type.equalsIgnoreCase("l")) {
			return Long.valueOf((long) Double.valueOf(value.toString())
					.doubleValue());
		}
		...
		if (type.equalsIgnoreCase("float")
			...
			return Float.valueOf((float) Double.valueOf(value.toString())
					.doubleValue());
		}
		...
		if (type.equalsIgnoreCase("boolean")
				...
				|| type.equalsIgnoreCase("b") || type.equalsIgnoreCase("bool")) {
			try {
		...
		if (type.equalsIgnoreCase("byte")
				...
				|| type.equalsIgnoreCase("bt")) {
			try {
				return (byte) (int) Double.valueOf(value.toString())
						.doubleValue();
			} catch (Exception e) {
		...
		if (type.equalsIgnoreCase("string")
				|| type.equalsIgnoreCase("java.lang.String")
				|| type.equalsIgnoreCase("s") || type.equalsIgnoreCase("str")) {
	...
	public static boolean isSameType(String type1, String type2) {
		...
		return type1.toLowerCase().endsWith(".".concat(type2.toLowerCase()))
				|| type2.toLowerCase()
						.endsWith(".".concat(type1.toLowerCase()))
				|| type1.equals(type2);
	}
	...
	public boolean isInstance(Object o, Class<?> c) {
		if (c.isInstance(o)) {
			...
			for (int y = 0; y < o.getClass().getClasses().length; y++) {
				if (isSameType(o.getClass().getClasses()[y].getName(), c
						.getName())) {
					return true;
				}
			}
	...
	static {
		...
		syntaxJ.setElseIf(Pattern
				.compile("\\}\\s*else\\s+if\\s*([\\s+|[\\s*\\(]][^\\{]+)\\{"));
		syntaxJ.setElse(Pattern.compile("\\}\\s*else\\s*\\{"));
		syntaxJ.setIfEnd(Pattern.compile("\\}"));
		syntaxJ.setLoop(Pattern
				.compile("while\\s*([\\s+|[\\s*\\(]][^\\{]+)\\{"));
		...
		syntaxB.setIf(Pattern
				.compile("^[\\s+i|i]f\\s*([\\s+|[\\s*\\(]].+)\\s*"));
		syntaxB.setElseIf(Pattern
				.compile("^[\\s+e|e]lseif\\s*([\\s+|[\\s*\\(]].+)\\s*"));
		syntaxB.setElse(Pattern.compile("^[\\s+e|e]ls[e\\s+|e]$"));
		syntaxB.setIfEnd(Pattern.compile("^[\\s+e|e]n[d\\s+|d]$"));
		syntaxB.setLoop(Pattern
				.compile("^[\\s+w|w]hile\\s*([\\s+|[\\s*\\(]].+)\\s*"));
		syntaxB.setLoopEnd(Pattern.compile("^[\\s+e|e]n[d\\s+|d]$"));
		syntaxB.setTry(Pattern
				.compile("^[\\s+t|t]ry\\s*([\\s+|[\\s*\\(]].+)\\s*"));
		...
		syntaxB.setFunction(Pattern
				.compile("^[\\s+f|f]unction\\s*([\\s+|[\\s*\\(]].+)\\s*"));
