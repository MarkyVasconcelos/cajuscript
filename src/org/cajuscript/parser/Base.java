...
public class Base implements Element, java.io.Serializable, Cloneable {
	...
	protected boolean canElementReturn(Element element) {
       return (!(element instanceof Command) && !(element instanceof Operation) && !(element instanceof Variable));
    }
	...
	public Value execute(CajuScript caju, Context context, Syntax syntax)
		...
		for (Element element : elements) {
			v = element.execute(caju, context, syntax);
	 ...
	 *            Script to be parsed
	 * @param syntax
	 *            Syntax style of the script
	 * @throws org.cajuscript.CajuScriptException
	 *             Errors ocurred on parsing
	 */
	public void parse(CajuScript caju, String script, Syntax syntax)
			throws CajuScriptException {
	...
	private void parse(Element base, CajuScript caju, LineDetail lineDetail,
			String script, Syntax syntax) throws CajuScriptException {
		if (base == null) {
			base = this;
		}
		...
		for (int y = 0; y < lines.length; y++) {
			...
			if ((syntaxPosition = syntax.matcherPosition(line, syntax
					.getLabel())).getStart() > -1) {
				label = line.substring(0, syntaxPosition.getStart()).trim();
				line = line.substring(syntaxPosition.getEnd()).trim();
			}
			if ((syntaxPosition = syntax.matcherPosition(line, syntax
					.getReturn())).getStart() == 0) {
				if (syntax.matcherEquals(line, syntax.getReturn())) {
					base.addElement(new Return(lineDetail));
				} else {
					Return r = new Return(lineDetail);
					r.setValue(evalValue(base, caju, lineDetail, syntax, line
							.substring(syntaxPosition.getEnd())));
					base.addElement(r);
				}
			} else if ((syntaxPosition = syntax.matcherPosition(line, syntax
					.getIf())).getStart() == 0) {
				...
				for (int z = y + 1; z < lines.length; z++) {
					...
					if (ifLevel == 0
							&& (syntaxPositionElseIf = syntax.matcherPosition(
									scriptIFline, syntax.getElseIf()))
									.getStart() == 0) {
						...
						continue;
							...
							&& (syntaxPositionElseIf = syntax.matcherPosition(
									scriptIFline, syntax.getElse())).getStart() == 0) {
						ifsConditions.add(scriptIFCondition);
						ifsStatements.add(scriptIF.toString());
					...
					scriptIF
							.append(originalIFline + CajuScript.SUBLINE_LIMITER);
				}
				if (ifLevel != 0 || !ifClosed) {
					throw CajuScriptException
							.create(caju, caju.getContext(),
									"\"If\" statement sintax error, maybe any \"if\" statement was not closed.");
				}
				...
				for (int i = 0; i < ifsConditions.size(); i++) {
					...
					var.setValue(evalValue(var, caju, lineDetail, syntax,
							_ifCondition));
				...
				for (int z = y + 1; z < lines.length; z++) {
					...
					scriptLOOP.append(originalLOOPline
							+ CajuScript.SUBLINE_LIMITER);
				}
				if (loopLevel != 0 || !loopClosed) {
					throw CajuScriptException
							.create(caju, caju.getContext(),
									"\"Loop\" statement sintax error, maybe any \"loop\" statement was not closed.");
				}
				...
				for (int z = y + 1; z < lines.length; z++) {
					...
					if (syntax.matcherPosition(scriptFUNCline,
							syntax.getFunction()).getStart() == 0) {
						funcLevel++;
				...
				if (funcLevel != 0 || !funcClosed) {
					throw CajuScriptException
							.create(
									caju,
									caju.getContext(),
									"\"Function\" statement sintax error, maybe any \"function\" statement was not closed.");
				}
				...
				for (int z = y + 1; z < lines.length; z++) {
					...
					if (tryLevel == 0
							&& syntax.matcherPosition(scriptTRYCATCHline,
									syntax.getTryCatch()).getStart() == 0) {
						isTry = false;
						isCatch = true;
						isFinally = false;
						continue;
							...
							&& syntax.matcherPosition(scriptTRYCATCHline,
									syntax.getTryFinally()).getStart() == 0) {
						isTry = false;
						isCatch = false;
						isFinally = true;
						continue;
				...
				if (tryLevel != 0 || !tryClosed) {
					throw CajuScriptException
							.create(caju, caju.getContext(),
									"\"Try\" statement sintax error, maybe any \"try\" statement was not closed.");
				}
				...
				parse(_finally, caju, lineDetail, scriptFINALLY.toString(),
						syntax);
				...
				base.addElement(tryCatch);
					...
					.getOperatorEqual())).getStart() > 0) {
				try {
					int p = syntaxPosition.getStart();
					String keys = line.substring(0, p);
					String value = line.substring(syntaxPosition.getEnd(), line
							.length());

					SyntaxPosition syntaxPositionOperator = syntax
							.lastOperatorMathematic(keys);
					if (syntaxPositionOperator.getEnd() == keys.length()) {
						p = syntaxPositionOperator.getStart();
					}
					String[] allKeys = keys.substring(0, p).replaceAll(" ", "")
							.split(",");
					for (String key : allKeys) {
						...
						if (syntaxPositionOperator.getOperator() != null) {
							...
							operation.setCommands(v, syntaxPositionOperator
									.getOperator(), evalValue(base, caju,
									lineDetail, syntax, value));
							var.setValue(operation);
						} else {
							var.setValue(evalValue(base, caju, lineDetail,
									syntax, value));
						}
						base.addElement(var);
					}
					...
					throw CajuScriptException.create(caju, caju.getContext(),
							"Incorrect definition", e);
				}
			} else if ((syntaxPosition = syntax.matcherPosition(line, syntax
					.getImport())).getStart() == 0) {
				String path = line.substring(syntaxPosition.getEnd()).trim();
				Import i = new Import(lineDetail);
				i.setPath(path);
				base.addElement(i);
			} else if ((syntaxPosition = syntax.matcherPosition(line, syntax
					.getContinue())).getStart() == -1
					&& (syntaxPosition = syntax.matcherPosition(line, syntax
							.getBreak())).getStart() == 0) {
	...
	private LineDetail loadLineDetail(String line) {
		if (line.startsWith(CajuScript.LINE_DETAIL_START)) {
			...
			int lineNumber = Integer.parseInt(line.substring(
					CajuScript.LINE_DETAIL_START.length(), indexLineInfoEnd));
			line = line.substring(indexLineInfoEnd
					+ CajuScript.LINE_DETAIL_END.length());
	...
	private boolean isStatementBegins(String line, Syntax syntax) {
		if (syntax.matcherPosition(line, syntax.getIf()).getStart() == 0) {
			...
			return true;
		} else if (syntax.matcherPosition(line, syntax.getFunction())
				.getStart() == 0) {
			return true;
		}
		return false;
	...
	private Element condition(Element base, CajuScript caju,
			LineDetail lineDetail, Syntax syntax, String script)
			throws CajuScriptException {
		try {
			script = script.trim();
			SyntaxPosition syntaxPositionLogical = syntax
					.firstOperatorLogical(script);
			if (syntaxPositionLogical.getStart() > -1) {
				Operation o = new Operation(lineDetail);
				o.setCommands(condition(base, caju, lineDetail, syntax, script
						.substring(0, syntaxPositionLogical.getStart())),
						syntaxPositionLogical.getOperator(), condition(base,
								caju, lineDetail, syntax, script
										.substring(syntaxPositionLogical
												.getEnd())));
				return o;
			} else if (script.length() != 0) {
				SyntaxPosition syntaxPosition = syntax
						.firstOperatorConditional(script);
				if (syntaxPosition.getStart() > -1) {
					Element e1 = evalValue(base, caju, lineDetail, syntax,
							script.substring(0, syntaxPosition.getStart()));
					Element e2 = evalValue(base, caju, lineDetail, syntax,
							script.substring(syntaxPosition.getEnd()));
					Operation o = new Operation(lineDetail);
	...
	private Element evalValue(Element base, CajuScript caju,
			LineDetail lineDetail, Syntax syntax, String script)
			throws CajuScriptException {
		return evalValueGroup(base, caju, lineDetail, syntax, script);
	}

	private Element evalValueSingle(Element base, CajuScript caju,
			LineDetail lineDetail, Syntax syntax, String script)
			throws CajuScriptException {
		try {
			script = script.trim();
			SyntaxPosition firstOperator = syntax
					.firstOperatorMathematic(script);
			if (firstOperator.getStart() > -1
					&& !syntax.matcherEquals(script, syntax.getNumber())) {
				if (firstOperator.getStart() == 0) {
					firstOperator = syntax.firstOperatorMathematic(script
							.substring(firstOperator.getEnd()));
				}
				SyntaxPosition priorityOperator = syntax.firstOperator(script,
						syntax.getOperatorMultiplication(), syntax
								.getOperatorDivision(), syntax
								.getOperatorModules());
				if (priorityOperator.getStart() > firstOperator.getStart()) {
					SyntaxPosition syntaxOperator1 = syntax
							.lastOperatorMathematic(script.substring(0,
									priorityOperator.getStart()));
					String script1 = syntaxOperator1.getStart() > -1 ? script
							.substring(0, syntaxOperator1.getStart()) : "";
					Operator operator1 = syntaxOperator1.getOperator();
					String scriptValue1 = script.substring(syntaxOperator1
							.getEnd() > -1 ? syntaxOperator1.getEnd() : 0,
							priorityOperator.getStart());
					Operation.Operator operator = priorityOperator
							.getOperator();
					SyntaxPosition syntaxOperator2 = syntax
							.firstOperatorMathematic(script
									.substring(priorityOperator.getEnd()));
					String scriptValue2 = script.substring(priorityOperator
							.getEnd(),
							syntaxOperator2.getStart() > -1 ? syntaxOperator2
									.getStart() : script.length());
					Operator operator2 = syntaxOperator2.getOperator();
					String script2 = syntaxOperator2.getEnd() > -1 ? script
							.substring(syntaxOperator2.getEnd()) : "";
					...
					if (operator1 != null) {
						...
						o1.setCommands(evalValueSingle(base, caju, lineDetail,
								syntax, script1), operator1, o);
						if (operator2 == null) {
							return o1;
						}
						e1 = o1;
					...
					o2.setCommands(e1, operator2, evalValueSingle(base, caju,
							lineDetail, syntax, script2));
					return o2;
				}
				Command value1 = new Command(lineDetail);
				value1
						.setCommand(script.substring(0, firstOperator
								.getStart()));
				String scriptFinal = script.substring(firstOperator.getEnd());
				SyntaxPosition value2Operator = syntax
						.firstOperatorMathematic(scriptFinal);
				if (value2Operator.getStart() > -1) {
					Command value2 = new Command(lineDetail);
					value2.setCommand(scriptFinal.substring(0, value2Operator
							.getStart()));
					...
					String varParamKey = CajuScript.CAJU_VARS_MATH.concat(
							caju.nextVarsCounter()).concat(
							Long.toString(varsMathCounter));
					varsMathCounter++;
					Variable varParam = new Variable(lineDetail);
					varParam.setKey(varParamKey);
					varParam.setValue(o);
					base.addElement(varParam);
					return evalValueSingle(base, caju, lineDetail, syntax,
							varParamKey.concat(scriptFinal
									.substring(value2Operator.getStart())));
				} else {
					Operation operation = new Operation(lineDetail);
					Command value2 = new Command(lineDetail);
					value2.setCommand(script.substring(firstOperator.getEnd()));
					operation.setCommands(value1, firstOperator.getOperator(),
							value2);
					return operation;
				}
			} else {
	...
	private Element evalValueGroup(Element base, CajuScript caju,
			LineDetail lineDetail, Syntax syntax, String script)
			throws CajuScriptException {
		...
		SyntaxPosition syntaxPosition = null;
		if ((syntaxPosition = syntax.matcherPosition(script, syntax
				.getFunctionCall())).getStart() > -1) {
			String varKey = CajuScript.CAJU_VARS_GROUP.concat(
					caju.nextVarsCounter()).concat(
					Long.toString(varsGroupCounter));
			varsGroupCounter++;
			Variable var = new Variable(lineDetail);
			var.setKey(varKey);
			Command c = new Command(lineDetail);
			String cmd = syntaxPosition.getGroup();
			String functionName = cmd.substring(0, syntax.matcherPosition(cmd,
					syntax.getFunctionCallParametersBegin()).getStart());
			SyntaxPosition syntaxFixOperator = syntax
					.lastOperatorLogical(functionName);
			if (syntaxFixOperator.getEnd() > -1) {
				syntaxPosition.setStart(syntaxPosition.getStart()
						+ syntaxFixOperator.getEnd());
			...
			String cmdBase = cmd;
			SyntaxPosition syntaxParameterBegin = syntax.matcherPosition(
					cmdBase, syntax.getFunctionCallParametersBegin());
			SyntaxPosition syntaxParameterEnd = syntax.matcherPosition(cmdBase,
					syntax.getFunctionCallParametersEnd());
			if (syntaxParameterBegin.getStart() > -1
					&& syntaxParameterBegin.getStart() < syntaxParameterEnd
							.getStart()) {
				String params = cmdBase.substring(
						syntaxParameterBegin.getEnd(), syntaxParameterEnd
								.getStart());
				cmd = cmd.substring(0, syntaxParameterBegin.getEnd());
				while (true) {
					SyntaxPosition syntaxPositionParam = syntax
							.matcherPosition(params, syntax
									.getFunctionCallParametersSeparator());
					...
					if (params.trim().length() != 0) {
						...
						String varParamKey = CajuScript.CAJU_VARS_GROUP.concat(
								caju.nextVarsCounter()).concat(
								Long.toString(varsGroupCounter));
						varsGroupCounter++;
						Variable varParam = new Variable(lineDetail);
						varParam.setKey(varParamKey);
						varParam.setValue(evalValueGroup(base, caju,
								lineDetail, syntax, params.substring(0,
										lenParamSeparatorStart)));
						base.addElement(varParam);
						if (syntaxPositionParam.getStart() == -1) {
							cmd = cmd.concat(varParamKey);
						} else {
							cmd = cmd.concat(varParamKey).concat(
									params.substring(lenParamSeparatorStart,
											lenParamSeparatorEnd));
						}
				...
				cmd = cmd.concat(cmdBase.substring(syntaxParameterEnd
						.getStart()));
			}
			c.setCommand(cmd);
			var.setValue(c);
			base.addElement(var);
			return evalValueGroup(base, caju, lineDetail, syntax, script
					.replace((CharSequence) cmdBase, (CharSequence) varKey));
		} else if ((syntaxPosition = syntax.matcherPosition(script, syntax
				.getGroup())).getStart() > -1) {
			String varKey = CajuScript.CAJU_VARS_GROUP.concat(
					caju.nextVarsCounter()).concat(
					Long.toString(varsGroupCounter));
			...
			Variable var = new Variable(lineDetail);
			var.setKey(varKey);
			var.setValue(evalValue(base, caju, lineDetail, syntax,
					syntaxPosition.getGroup()));
			base.addElement(var);
			return evalValueGroup(base, caju, lineDetail, syntax, script
					.replace((CharSequence) syntaxPosition.getAllContent(),
							(CharSequence) varKey));
		} else {
			if ((syntaxPosition = syntax.firstOperatorConditional(script))
					.getStart() > -1
					|| (syntaxPosition = syntax.firstOperatorLogical(script))
							.getStart() > -1) {
